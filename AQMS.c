...
int led1 = 9;  //led1 connected to pin 9
int led2 = 10; //led2 connected to pin 10
...
void setup() 
{
  ...
  pinMode (led1,OUTPUT);      // led1 is connected as output from Arduino
  pinMode (led2,OUTPUT);     // led2 is connected as output from Arduino
  ...
  if (ppm > threshold)            // check is ppm is greater than threshold or not
    {
      ...
      tone(led1,1000,200);         //blink led with turn on time 1000mS, turn off time 200mS
      digitalWrite(buz,HIGH);     //Turn ON Buzzer
      tone(led2,200,1000);     
    }
    else
    {
      digitalWrite(led1,LOW);   //jump here if ppm is not greater than threshold and turn off LED
